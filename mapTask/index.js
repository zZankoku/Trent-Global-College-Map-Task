...
L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
    ...
    tileSize: 512,
    zoomOffset: -1,
    accessToken: 'pk.eyJ1IjoiemFjLXRyZW50LXByb2plY3QiLCJhIjoiY2tyeWVxY3hoMHZhdDJ1bWsxN2RyeXp1dCJ9.GDOn61ZT9XXWMknAG9-KZA'
}).addTo(map);
...
window.addEventListener('DOMContentLoaded', async ()=>{
    ...
    const MARKET_RESPONSE = await axios.get('https://data.gov.sg/api/action/datastore_search?resource_id=8f6bba57-19fc-4f36-8dcf-c0bda382364d&limit=250');
    ...
    for (index in response.data.features) {
        ...
        for (market_index in market_data) {
            ...
            if (postal_re.test(market["location_of_centre"])) {
                ...
                response.data.features[index].properties.title = market["name_of_centre"];
